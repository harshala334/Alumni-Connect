...
import {
  ...
  FaUser,
  FaCalendar,
  FaCamera,FaBullhorn,FaTrophy // Added icons
} from 'react-icons/fa';
...
function Topbar() {
  ...
  const [whatsNewDropdownOpen, setWhatsNewDropdownOpen] = useState(false); // Added state
  const [mentorshipDropdownOpen, setMentorshipDropdownOpen] = useState(false); // Added state
  ...
  const toggleEngageDropdown = () => {
    ...
    setConnectDropdownOpen(false); // Close other dropdowns
    setWhatsNewDropdownOpen(false); // Close other dropdowns
    setMentorshipDropdownOpen(false); // Close other dropdowns
  };

  const toggleConnectDropdown = () => {
    setConnectDropdownOpen(!connectDropdownOpen);
    setEngageDropdownOpen(false); // Close other dropdowns
    setWhatsNewDropdownOpen(false); // Close other dropdowns
    setMentorshipDropdownOpen(false); // Close other dropdowns
  };
          ...
          {mentorshipDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg z-10">
              <Link
                ...
                className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
              >
                <FaStar className="mr-2 inline-block" /> Become a Mentor
              </Link>
              <Link
                to="/search-people"
                className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
